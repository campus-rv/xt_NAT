...
static inline uint32_t get_nat_addr(uint32_t addr)
{
  return htonl(ntohl(nat_pool_start) + reciprocal_scale(jhash_1word(addr, 0), get_pool_size()));
}

static inline uint32_t get_hash_nat_ent(uint8_t proto, uint32_t addr, uint16_t port)
{
  return reciprocal_scale(jhash_3words((u32) proto, addr, (u32) port, 0), nat_hash_size);
}

static inline uint32_t get_hash_user_ent(uint32_t addr)
{
...
void users_htable_remove(void)
{
  ...
  for (i = 0; i < users_hash_size; i++)
  {
    ...
    if (ht_users[i].use)
      printk(KERN_ERR "xt_NAT users_htable_remove ERROR: bad use value: %d in element %d\n", ht_users[i].use, i);
...
void nat_htable_remove(void)
{
  ...
  for (i = 0; i < nat_hash_size; i++)
  {
    ...
    if (ht_inner[i].use)
      printk(KERN_ERR "xt_NAT nat_htable_remove inner ERROR: bad use value: %d in element %d\n",
        ht_inner[i].use, i);
  ...
  for (i = 0; i < nat_hash_size; i++)
  {
    ...
    if (ht_outer[i].use)
      printk(KERN_ERR "xt_NAT nat_htable_remove outer ERROR: bad use value: %d in element %d\n",
        ht_outer[i].use, i);
...
static int check_user_limits(const uint8_t proto, const uint32_t addr)
{
  struct user_htable_ent *user;
  struct hlist_head *head;
  uint32_t hash;
...
void update_user_limits(const uint8_t proto, const uint32_t addr, const int8_t operation)
{
  ...
  {
    // printk(KERN_DEBUG "xt_NAT update_user_limits: %pI4 not found\n", &addr);
...
static void netflow_export_flow_v5(const uint8_t proto, const uint32_t useraddr, const uint16_t userport,
    const uint32_t nataddr, const uint16_t natport, const int flags)
{
  ...
  rec->s_port = userport;
  rec->d_port = natport;
  // rec->reserved	= 0; /* pdu is always zeroized for v5 in
  // netflow_switch_version */
...
struct nat_htable_ent *create_nat_session(const uint8_t proto, const uint32_t useraddr, const uint16_t userport, const uint32_t nataddr)
{
  ...
  {
    printk(KERN_NOTICE "xt_NAT: %pI4 exceeded max allowed sessions\n", &useraddr);
    return NULL;
  }
  ...
  if (likely(proto == IPPROTO_TCP || proto == IPPROTO_UDP || proto == IPPROTO_ICMP))
  {
    ...
    if (natport == 0)
    {
      printk(KERN_WARNING "xt_NAT create_nat_session ERROR: Not found free nat "
  ...
  //   &session2->data->in_addr, ntohs(session2->data->in_port), &session2->addr, ntohs(session2->port));
  rcu_read_lock_bh();
  return lookup_session(ht_outer, proto, nataddr, natport);
}

static unsigned int nat_tg(struct sk_buff *skb, const struct xt_action_param *par)
{
  struct iphdr *ip;
  struct tcphdr *tcp;
  struct udphdr *udp;
  ...
  {
    printk(KERN_DEBUG "xt_NAT DEBUG: Drop version %d packet\n", ip->version);
    return NF_DROP;
  }
  ...
  else if (info->variant == XTNAT_DNAT)
  {
    ...
    else if (ip->protocol == IPPROTO_ICMP)
    {
      ...
      else if (icmp->type == 3 || icmp->type == 4 || icmp->type == 5 || icmp->type == 11 || icmp->type == 12 || icmp->type == 31)
      {
        ...
        {
          printk(KERN_DEBUG "xt_NAT DNAT: Drop related ICMP packet with truncated IP header\n");
          return NF_DROP;
        }
        ...
        if (ip->protocol == IPPROTO_TCP)
        {
          ...
          {
            printk(KERN_DEBUG "xt_NAT DNAT: Drop related ICMP packet witch "
        ...
        else if (ip->protocol == IPPROTO_ICMP)
        {
          if (skb->len < ip_hdrlen(skb) + sizeof(struct icmphdr) + sizeof(struct iphdr) + 8)
          {
            printk(KERN_DEBUG "xt_NAT DNAT: Drop related ICMP packet with truncated ICMP header\n");
            return NF_DROP;
          }
...
void users_cleanup_timer_callback(struct timer_list *timer)
{
  ...
  if (ht_users == NULL)
  {
    printk(KERN_ERR "xt_NAT USERS CLEAN ERROR: Found null ptr for ht_users\n");
...
void sessions_cleanup_timer_callback(struct timer_list *timer)
{
  ...
  {
    printk(KERN_ERR "xt_NAT SESSIONS CLEAN ERROR: Found null ptr for ht_inner/ht_outer\n");
    ...
    if (ht_inner[i].use > 0)
    {
      ...
      hlist_for_each_entry_safe(session, next, head, list_node)
      {
        ...
        if (session->data->timeout == 0)
          netflow_export_flow_v5(session->proto, session->addr, session->port,
            get_nat_addr(session->addr), session->data->out_port, 1);
...
static int add_nf_destinations(const char *ptr)
{
  ...
  for (; ptr; ptr += len)
  {
    ...
    if (!succ)
    {
      printk(KERN_WARNING "xt_NAT: can't parse netflow destination: %.*s\n", len, ptr);
      continue;
    }
